...
server.get("/count", async (request, response) => {
  ...
  if (str.indexOf(",") !== -1) {
    ...
    if (
      str.indexOf(" OR") > 0 &&
      str.indexOf(" OR") != str.lastIndexOf(" OR")
    ) {
      // tests for >1 character group in string
      str = str.slice(0, str.lastIndexOf(" OR")); // lops off the last comma
      str =
        str.slice(0, str.lastIndexOf(" OR")) +
        " and " +
        str.substring(str.lastIndexOf(" OR") + 1);
    }
    str = str + ")";
  }
